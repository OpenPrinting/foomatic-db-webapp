...
class DriverSupportContact
{
  ...
  public function __construct($driver_id, $data) {
    ...
    if ($data) {
      $this->translation["description"] = new Translation($data, "driver_support_contact", ["driver_id" => $this->driver_id, "url" => $this->url, "level" => $this->level], "description");
    }
  }
  ...
  public function loadDB($driver_id, $url, $level, OPDB $db = null) {
    ...
    $url = $db->mysqli_real_escape_string($url);
    $level = $db->mysqli_real_escape_string($level);
    ...
    $this->translation["description"] = new Translation(null, "driver_support_contact", ["driver_id" => $this->driver_id, "url" => $this->url, "level" => $this->level], "description");
    $this->translation["description"]->loadDB("driver_support_contact", ["driver_id" => $this->driver_id, "url" => $this->url, "level" => $this->level], "description", $db);
  ...
  public function saveDB(OPDB $db = null) {
    ...
    if ($count) {
      ...
      $query .= "\"".$db->mysqli_real_escape_string($this->url)."\",";
      $query .= "\"".$db->mysqli_real_escape_string($this->level)."\",";
      $query .= "\"".$db->mysqli_real_escape_string($this->description)."\")";
    }
