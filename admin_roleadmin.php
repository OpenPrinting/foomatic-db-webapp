...
if(!empty($_REQUEST['roleID']) && is_numeric($_REQUEST['roleID'])) 
{
  ...
  else 
  {
		...
		if($role->isValid()) 
    {
      ...
      $SMARTY->assign('pagesize',$rpagesize);
      $SMARTY->assign('membertotal',$role->countMembers());
      $SMARTY->assign('members',$role->getSomeMembers($rpagesize,$offset));
