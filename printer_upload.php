<?php
  ...
  if (isset($_POST['submit'])) {
    ...
    if (strlen($error) > 0) {
      header('location: /printers/upload?msg=error&type=makemodel');
      exit(0);
    }
    ...
    if ($CONF->mailusername) {
      $mailbody = "Date: " . $today . "\n";
      $mailbody .= "User: " . $user . "\n";
      $mailbody .= "Uploaded a printer \n";
      $mailbody .= "Make: " . $make . "\n";
      $mailbody .= "Model: " . $model . "\n\n";
      $mailbody .= "This email is auto-generated by OpenPrinting.org Printer submission.";
  ...
  if($SESSION->checkPermission('printer_noqueue')) {
    $SMARTY->assign('isTrusted',1);
  }
  ...
  $resDriver = $DB->query("SELECT id, name, execution, shortdescription, pj.count as printerCount
    FROM driver 
    LEFT JOIN 
  ...
  $driverselect = "<select tabindex=\"33\" name=\"dsnameNew[]\" id=\"dsnameNew###\">";
  $driverselect .= "<option value=\"\" selected=\"selected\">No driver</option>";
  
  foreach ($rD as $d) {
    $driverselect .= "<option value=\"{$d['id']}\">{$d['name']}</option>";
